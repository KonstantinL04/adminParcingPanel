...
const userRoles = computed(() => new Set(auth.user?.roles || []));
const isFullAdmin = computed(() => {
  const user = auth.user;
  if (!user) return false;
  return user.is_superuser || userRoles.value.has("admin");
});
const isModeratorOnly = computed(() => {
  const user = auth.user;
  if (!user) return false;
  return !isFullAdmin.value && (user.is_staff || userRoles.value.has("moderator"));
});
...
const canManageUsers = computed(() => {
  return isFullAdmin.value;
});
const canAssignRoles = computed(() => isFullAdmin.value);
  ...
  <div v-else class="app-wrapper">
    ...
    <nav class="navbar navbar-expand-lg navbar-custom border-bottom shadow-sm">
      <div class="container-fluid navbar-safe">
        ...
        <!-- logo -->
        <router-link to="/map-view" class="navbar-brand brand-title d-flex align-items-center gap-2">
          ...
          <ul class="navbar-nav mx-auto align-items-center gap-2 main-nav-center">
            ...
            <li class="nav-item dropdown main-dropdown">
              ...
              <ul class="dropdown-menu dropdown-menu-end custom-menu shadow border-0">
                <li>
                  <router-link to="/map-view" class="dropdown-item">
                    <i class="bi bi-map me-2"></i>
                    Карта событий
                  </router-link>
                </li>

                <li class="dropdown-submenu" v-if="!isModeratorOnly">
                  <a href="#" class="dropdown-item dropdown-submenu-toggle">
                    <span>
                  ...
                  <ul class="dropdown-menu custom-menu submenu-menu shadow border-0">
                    <li>
            ...
            <li class="nav-item" v-if="!isModeratorOnly">
              <router-link to="/driving-mode" class="nav-link top-link">
                <i class="bi bi-broadcast-pin me-2"></i>
                Режим езды
              </router-link>
            </li>

            <li class="nav-item" v-if="canModerate">
              <router-link to="/moderation" class="nav-link top-link">
            ...
            <!-- парсер -->
            <li class="nav-item dropdown main-dropdown" v-if="!isModeratorOnly">
              <a href="#" class="nav-link dropdown-toggle top-link" data-bs-toggle="dropdown">
            ...
            <li class="nav-item dropdown main-dropdown" v-if="canManageUsers">
              <a href="#" class="nav-link dropdown-toggle top-link" data-bs-toggle="dropdown">
              ...
              <ul class="dropdown-menu dropdown-menu-end custom-menu shadow border-0">
                ...
                <li v-if="canAssignRoles">
                  <router-link to="/user-roles" class="dropdown-item">
                ...
                <li>
                  <router-link to="/reputation-rules" class="dropdown-item">
